...
signed main() {
	...
	VEC_ROW(int, m, l, r);
	ST st(n, MAX);
	REP(i, n) st.update(i, t[i]);

	struct D {
		int l, r, i;
	};
	std::vector<D> d(m);
	REP(i, m) d[i] = D{--l[i], r[i], i};

	std::sort(ALL(d), [&](const D& l, const D& r)
		{
			PAIR pl(l.l, l.r), pr(r.l, r.r);
			if (pl.first <= pr.first && pr.second <= pl.second) return false;
			if (pr.first <= pl.first && pl.second <= pr.second) return true;

			if (pl < pr && pr.first < pl.second) {
				std::swap(pl.second, pr.first);
			}
			if (pr < pl && pl.first < pr.second) {
				std::swap(pr.second, pl.first);
			}
			return st.query(pl.first, pl.second) < st.query(pr.first, pr.second);
		});

	REP(i, m) {
		OUT(d[i].i + 1)SPBR(i, m);
	}
